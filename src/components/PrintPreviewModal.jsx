...
/**
 * Obtiene las promociones aplicadas a una venta.
 * tipo_venta_id: 1=producto, 2=servicio
 */
...
/**
 * Construye el HTML completo del ticket térmico para imprimir en ventana emergente.
 * Incluye el bloque de promociones aplicadas si existen.
 */
const buildTicketHTML = (venta, tipo, promociones = []) => {
  ...
  const totalPromos = promos.reduce((s, p) => s + parseFloat(p.monto_ahorrado || 0), 0);
  ...
  // ✅ HTML del bloque de promociones
...
const PrintPreviewModal = ({ venta, tipo, onClose }) => {
  ...
  const handleImprimir = async () => {
    if (formato === 'ticket') {
      // ✅ Pasamos las promociones al HTML de impresión
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 py-6">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[95vh] flex flex-col">
            ...
            return (
              <button key={f.id} onClick={() => setFormato(f.id)}
                className={`flex items-center gap-2 px-4 py-2 rounded-xl border-2 text-sm font-semibold transition-all ${
                  activo ? 'border-[#7B1FA2] bg-purple-50 text-[#7B1FA2]' : 'border-gray-200 text-gray-600 hover:border-gray-300 bg-white'
                }`}>
        ...
        <div className="flex-1 overflow-hidden bg-gray-200 relative min-h-0">
          ...
          {previewUrl && !loading && (
            <iframe id="pdf-preview-iframe" src={previewUrl} className="w-full h-full border-0" title="Vista previa del comprobante" />
          )}
          {!previewUrl && !loading && (
